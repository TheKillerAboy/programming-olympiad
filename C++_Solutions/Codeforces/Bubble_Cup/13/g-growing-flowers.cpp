...
int main(){
	...
	FOR(i,n){
		cin>>l>>d;
		if(events.find(l)==events.end()) events[l] = 0;
		if(events.find(d)==events.end()) events[d] = 0;
		events[l]++;
		events[d]--;
	}
	pii best = {0,0};
	...
	FORA(event, events){
		int year, amount;
		tie(year,amount) = event;
		current+=amount;
		best = max(best,{current,INT_MAX-year});
	}
	cout<<INT_MAX-best.second<<' '<<best.first<<'\n';
