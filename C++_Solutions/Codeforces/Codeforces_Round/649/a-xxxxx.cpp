...
int main(){
	...
	while(t--){
		cin>>n>>x;
		int lastNon = INT_MIN/2;
		FOR(i,n)cin>>arr[i];
		FORR(i,n) if(arr[i]%x!=0){lastNon=i;break;}
		int longest=0;
		suffixSum[n-1]=arr[n-1];
		FOR1(i,n) suffixSum[n-1-i] = suffixSum[n-i]+arr[n-1-i];
		FOR(i,n){
			if(suffixSum[i]%x==0){
				longest = max(longest,lastNon-i);
			}
			else longest = max(longest,n-i);
		}
		cout<<(longest==0?-1:longest)<<'\n';
	}
