...
long long t,n;
long long arr[2*SSIZE], arrc[2*SSIZE];
...
long long dup(long long first){
	long long out = 0;
	long long ori[2] = {arr[first],arr[first+1]};
	arr[first] = ori[0];
	arr[first+1] = ori[0];
	out = min(dec(),acc());
	arr[first] = ori[1];
	arr[first+1] = ori[1];
	out = min(out,min(dec(),acc()));
	arr[first] = ori[0];
	arr[first+1] = ori[1];
	return out;
}
...
int main(){
	...
	while(t--){
		...
		FOR(i,n) cin>>arr[i];
		pii max_ = {-1,-1};
		pii min_ = {LLONG_MAX,-1};
		FOR1(i,n) {
			max_ = max(max_, pii{arr[i-1]-arr[i],i});
			min_ = min(min_, pii{arr[i-1]-arr[i],i});
		}
		long long out = min(min(acc(),dec()),min(dup(max_.second-1),dup(min_.second-1)));
		cout<<out<<'\n';
