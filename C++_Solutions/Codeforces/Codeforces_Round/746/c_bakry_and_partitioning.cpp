...
int main(){
	...
	while(t--){
		...
		map<int,int> count;
		FOR(i,n-1){
			int val = nodes[edges[i].first]^nodes[edges[i].second];
			if(count.find(val) == count.end()) count[val] = 0;
			count[val]++;
		}
		pii max_ = {-1,-1};
		FORA(e,count) max_ = max(max_, pii{e.second,e.first});
		if(n-1-max_.first>k-1 || n-1-max_.first == 0) cout<<"NO\n";
		else cout<<"YES\n";
