...
int main(){
	...
	cin>>n;
	FOR(i,n) cin>>arr[i];
	FOR(i,n) pow_2[i] = max_pow_2(arr[i]);
	sort(pow_2,pow_2+n);
	pii max_ = {1,pow_2[0]};
	pii cur = {1,pow_2[0]};
	FOR1(i,n){
		max_ = max(max_,cur);
		if(pow_2[i]==pow_2[i-1]) cur = {cur.first+1,cur.second};
		else cur = {1,pow_2[i]};
		max_ = max(max_,cur);
	}
	// TRACEV(pow_2,pow_2+n);TRACE(max_);
	cout<<n-max_.first<<'\n';
	FOR(i,n){
		if(max_pow_2(arr[i])!=max_.second) cout<<arr[i]<<' ';
	}
