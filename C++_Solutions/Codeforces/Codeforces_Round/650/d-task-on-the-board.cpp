...
int main(){
	...
	while(t--){
		...
		FOR(i,26) amount[i] = 0;
		FOR(i,n) amount[s[i]-'a']++;
		FOR(i,n) letters.insert(s[i]);
		while(true){
			// TRACEV(arr,arr+m);_;TRACE(t_);
			bool break_ = true;
			FOR(i,m) if(arr[i]!=-1) break_=false;
			if(break_) break;
			FOR(i,m) changes[i] = 0;
			list<int> queue;
			FOR(i,m){
			...
			while(true){
				...
				if(amount[letter-'a']>=queue.size()){
					FORA(i,queue) t_[i] = letter;
					letters.erase(letters.begin());
					break;
				}
				letters.erase(letters.begin());
			}
			FOR(i,m) arr[i]+=changes[i];
		}
		cout<<t_<<'\n';
	}
