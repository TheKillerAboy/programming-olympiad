...
int t,n,k;
map<int,int> cur_primes;

void primes(int u){
	cur_primes.clear();
	FORS(2,i,floor(sqrt(u))+1){
		while(u%i==0){
			if(cur_primes.find(i)==cur_primes.end())cur_primes[i]=0;
			++cur_primes[i];
			u/=i;
		}
	}
	if(u>1){
		if(cur_primes.find(u)==cur_primes.end())cur_primes[u]=0;
		++cur_primes[u];
	}
}
...
int main(){
	...
	while(t--){
		...
		int i = 0;
		set<int> cur = {1};
		FORA(prime, cur_primes){
			set<int> cop = cur;
			int cur_pow = 1;
			FORI(i,prime.second){
				cur_pow *= prime.first;
				FORA(val, cop){
					cur.insert(val*cur_pow);
				}
			}
		}
		auto it = prev(cur.upper_bound(k));
		cout<<n/(*it)<<'\n';
