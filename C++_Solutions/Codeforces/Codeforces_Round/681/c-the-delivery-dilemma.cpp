...
int main(){
	...
	while(t--){
		...
		sort(ab,ab+n);
		suffixsum[n-1] = ab[n-1].second;
		FOR1(i,n){
			suffixsum[n-1-i] = suffixsum[n-i] + ab[n-1-i].second;
		}
		long long min_ = LLONG_MAX;
		FORS(-1,i,n){
			long long max_ = i==-1?0:ab[i].first;
			long long suffix = i+1==n?0:suffixsum[i+1];
			min_ = min(min_, max(max_,suffix));
		}
		cout<<min_<<'\n';
	}
