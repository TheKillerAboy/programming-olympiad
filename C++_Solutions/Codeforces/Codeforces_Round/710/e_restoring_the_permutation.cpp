...
int main(){
	...
	while(t--){
...

		{
			FOR(i,n) out[i] = 0;
			set<int> avalible;
			FORI(i,n) avalible.insert(avalible.end(),i);
			out[0] = arr[0];
			FOR1(i,n) if(arr[i]!=arr[i-1]) out[i] = arr[i];
			FOR(i,n) avalible.erase(arr[i]);
			int cur_max = out[0];
			FOR(i,n){
				if(out[i] == 0){
					out[i] = *avalible.begin();
					avalible.erase(out[i]);
				}
				else{
					cur_max = out[i];
				}
		...
		}
		{
			...
			FOR(i,n){
				if(out[i] == 0){
					out[i] = *(--avalible.lower_bound(cur_max));
					avalible.erase(out[i]);
				}
				else{
					cur_max = out[i];
