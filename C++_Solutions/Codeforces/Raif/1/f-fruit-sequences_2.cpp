...
int main(){
	...
	long long sum = 0;
	cin>>n>>s;
	list<long long> segments = {1};
	FOR1(i,n) if(s[i]==s[i-1]) (*segments.rbegin())++;
	else segments.push_back(1);
	bool curOne = s[0]=='1';
	long long out = 0;
	long long index = 0;
	FOR(i,5*SSIZE) histo[i] = 0;
	FORA(seg,segments){
		if(!curOne) out += seg*sum;
		else{
			FORI(size, seg){
				sum += index+size - histo[size];
				out += sum;
				histo[size] = index+seg-size+1;
			}
		}
		curOne=!curOne;
		index+=seg;
