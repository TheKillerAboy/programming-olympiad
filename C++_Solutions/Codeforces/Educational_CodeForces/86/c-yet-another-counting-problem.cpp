...
long long prevsum(long long l,long long r) {
	if(r<b) return 0;
	else return r-max(b,l)+1;
}
...
int main(){
	...
	while(t--){
		...
		if(b<a)swap(a,b);
		long long total = prevsum(0,lcm-1);
		while(q--){
			cin>>l>>r;
			long long out = 0;
			while(l<=r){
				if(l%lcm==0&&(r-l)/lcm>0){
					out += ((r-l)/lcm)*total;
					l += ((r-l)/lcm)*lcm;
				}
				else{
					long long r_ = min(r,(l/lcm)*lcm + lcm-1);
					out += prevsum(l%lcm,r_%lcm);
					l = r_ + 1; 
				}
			}
			cout<<out<<' ';
