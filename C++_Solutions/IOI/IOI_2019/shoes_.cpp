...
int main(){
	...
	while(!inlist.empty()){
		...
		shoes_indexed[shoes[currentShoe]].pop_front();

		int nextShoe = shoes_indexed[-shoes[currentShoe]].front();
		shoes_indexed[-shoes[currentShoe]].pop_front();
		inlist.erase(inlist.find(nextShoe));
		removed.insert(nextShoe);

		ll dif = nextShoe - currentShoe - 1 - (removed.order_of_key(nextShoe) - removed.order_of_key(currentShoe+1));
